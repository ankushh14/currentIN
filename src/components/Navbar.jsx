        ...
        <>
           <nav className="flex justify-between items-center bg-black w-full  p-4">
            ...
            <div className="bars flex flex-col lg:hidden cursor-pointer" onClick={()=>{setNav(!Nav);}}>
                <span className={`lines  border border-white m-1 w-6 transition-all duration-500 ${!Nav?"rotate-45 translate-y-2.5":""}`}></span>
                <span className={`lines  border border-white m-1 w-6 transition-all duration-500 ${!Nav?"-rotate-45":""}`}></span>
                <span className={`lines border border-white m-1 w-6 transition-all ${!Nav?"hidden -translate-y-2":""}`}></span>
            </div>
            
            <ul className={`nav-list z-50 absolute bg-black  p-4 top-[4.5rem] flex flex-col w-screen ${!Nav?"left-0":"-left-full"} transition-all duration-500 lg:transition-none lg:static lg:flex lg:flex-row lg:w-fit lg:p-0 lg:border-none`}>
                <NavLink to = "/">
